...
main(int argc, const char* argv[])
{
    ...
    for(i = 0 ; args->scripts[i] ; i++) {
        FILE *fp = fopen(args->scripts[i], "r");
        if (fp != NULL) {
            if (fseek(fp, 0L, SEEK_END) == 0) {
                slen = ftell(fp);
                if (slen == -1) { /* Error */ }
                scriptsrc = (char *)malloc(sizeof(char) * (slen + 1));
                if (fseek(fp, 0L, SEEK_SET) != 0) { /* Error */ }
                size_t newLen = fread((void *)scriptsrc, sizeof(char), slen, fp);
                if ( ferror( fp ) != 0 ) {
                    fputs("Error reading file", stderr);
                } else {
                    scriptsrc[newLen++] = '\0'; /* Just to be safe. */
                }
            }
            fclose(fp);
        }
